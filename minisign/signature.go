...
//
// A signature is generated when signing a message with the wallet's Ed25519
// signing key:
//
//	signature, err := Sign(privateKey, message)
//
// The signature of a message can then be verified with the corresponding
// public key:
//
//	if Verify(publicKey, message, signature) {
//	   // => signature is valid
//	   // => message has been signed with the corresponding private key
//	}
type Signature struct {
	...
	// KeyID may be the 64 bit ID of the signing key that was used to
	// produce this signature. It can be used to identify the corresponding
	...
	// TrustedComment is a comment that has been signed and is verified
	// during signature verification.
	TrustedComment string

	// UntrustedComment is a comment that has not been signed and is not
	...
	// Signature is the Ed25519 signature of the message that has been
	// signed.
	Signature [ed25519.SignatureSize]byte

	// CommentSignature is the Ed25519 signature of Signature concatenated
	// with the TrustedComment:
	//
...
//
// In contrast to MarshalText, String does not fail if s is not a valid
// minisign signature.
...
//
// It returns an error if s cannot be a valid signature, for example when
// s.Algorithm is neither EdDSA nor HashEdDSA.
