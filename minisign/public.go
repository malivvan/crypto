...
// PublicKeyFromEd25519 derives the minisign public key that corresponds to a
// wallet Ed25519 public key (for example the key returned by
// Wallet.SigningKey().Public()).
//
// The returned PublicKey verifies messages signed with the matching wallet
// signing key, and its textual representation interops with the reference
// minisign tool.
func PublicKeyFromEd25519(publicKey *ed25519.PublicKey) PublicKey {
	return PublicKey{
		id:    keyIDFromPublicKey(publicKey.Point),
		bytes: fromPoint(publicKey.Point),
	}
}

// PublicKey is a minisign public key.
//
// A public key is used to verify whether messages have been signed with the
// corresponding wallet signing key.
...
// Bytes returns the raw 32-byte Ed25519 public key point.
...
func fromPoint(point []byte) (key [ed25519.PublicKeySize]byte) {
	copy(key[:], point)
	return key
}
