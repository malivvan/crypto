...
import (
	...
	"github.com/malivvan/crypto/pgp/keywrap"
)
...
func Validate(pk *PrivateKey) (err error) {
	var expectedPublicKey, privateKey Key
	subtle.ConstantTimeCopy(1, privateKey[:], pk.Secret)
	KeyGen(&expectedPublicKey, &privateKey)
...
// The seed is the raw X25519 scalar, i.e. the same 32 bytes used by other
// x25519 implementations (e.g. SSH keys), which allows one seed to back keys
// in multiple ecosystems.
func GenerateKeyFromSeed(seed []byte) (*PrivateKey, error) {
	if len(seed) != KeySize {
		return nil, errors.InvalidArgumentError("x25519: the seed has the wrong size")
	}
	if constantTimeIsZero(seed) {
		return nil, errors.InvalidArgumentError("x25519: the seed is zero")
	}
	privateKeyOut := new(PrivateKey)
	var privateKey, publicKey Key
...
func generateKey(rand io.Reader, privateKey *Key, publicKey *Key) error {
	maxRounds := 10
	isZero := true
...
func Encrypt(rand io.Reader, publicKey *PublicKey, sessionKey []byte) (ephemeralPublicKey, encryptedSessionKey []byte, err error) {
	var ephemeralPrivate, ephemeralPublic, staticPublic, shared Key
	...
	// Compute shared key
	ok := Shared(&shared, &ephemeralPrivate, &staticPublic)
	if !ok {
...
func Decrypt(privateKey *PrivateKey, ephemeralPublicKey, ciphertext []byte) (encodedSessionKey []byte, err error) {
	var ephemeralPublic, staticPrivate, shared Key
	...
	// Compute shared key
	ok := Shared(&shared, &staticPrivate, &ephemeralPublic)
	if !ok {
		err = errors.KeyInvalidError("x25519: the ephemeral public key is a low order point")
		return
	}
	return DecryptWithSharedSecret(ephemeralPublicKey, privateKey.PublicKey.Point[:], shared[:], ciphertext)
}

// DecryptWithSharedSecret decrypts a session key stored in ciphertext with a
// shared secret that has already been computed. It allows decryption with
// private keys that are backed by external hardware (e.g. a YubiKey), which
// can compute the shared secret without exposing the private scalar.
func DecryptWithSharedSecret(ephemeralPublicKey, publicKey, sharedSecret, ciphertext []byte) (encodedSessionKey []byte, err error) {
	// Derive the encryption key from the shared secret
	encryptionKey := applyHKDF(ephemeralPublicKey, publicKey, sharedSecret)
