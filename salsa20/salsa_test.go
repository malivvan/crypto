...
func TestOutOfBoundsWrite(t *testing.T) {
	...
	defer func() {
		err := recover()
		if err == nil {
			t.Error("XORKeyStreamCounter expected to panic on len(dst) < len(src), but didn't")
		}
		if plainText[3] == '3' {
			t.Errorf("XORKeyStreamCounter did out of bounds write, want %v, got %v", want, string(plainText))
		}
	}()
	XORKeyStreamCounter(plainText[:3], cipherText, &counter, &key)
}
