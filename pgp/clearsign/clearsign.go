...
package clearsign // import "github.com/malivvan/crypto/pgp/clearsign"
...
var allowedHashHeaderValues = []string{"SHA1", "SHA256", "SHA512"}
...
func (b *Block) VerifySignature(keyring wallet.KeyRing, config *packet.Config) (signer *wallet.Entity, err error) {
	_, signer, err = wallet.VerifyDetachedSignature(keyring, bytes.NewBuffer(b.Bytes), b.ArmoredSignature.Body, config)
	return
}
...
func acceptableHashesToWrite(signingKey *packet.PublicKey) []crypto.Hash {
	// All supported signing algorithms (EdDSA over Ed25519 and Ed25519)
	// accept SHA-256 and SHA-512.
	return []crypto.Hash{
		crypto.SHA256,
		crypto.SHA512,
	}
