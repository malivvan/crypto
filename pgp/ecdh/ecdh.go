...
func Decrypt(priv *PrivateKey, vsG, c, curveOID, fingerprint []byte) (msg []byte, err error) {
	...
	return DecryptFromSharedSecret(zb, &priv.PublicKey, c, curveOID, fingerprint)
}

// DecryptFromSharedSecret decrypts an encrypted session key with a shared
// secret that has already been computed. It allows decryption with private
// keys that are backed by external hardware (e.g. a YubiKey), which can
// compute the shared secret without exposing the private scalar.
func DecryptFromSharedSecret(zb []byte, pub *PublicKey, c, curveOID, fingerprint []byte) (msg []byte, err error) {
	var m []byte
