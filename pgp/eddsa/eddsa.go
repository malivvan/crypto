...
// GenerateKeyFromSeed derives an EdDSA key from the given 32-byte seed.
// The seed is the raw RFC 8032 private key material, i.e. the same 32 bytes
// used by other ed25519 implementations (e.g. SSH keys).
func GenerateKeyFromSeed(seed []byte, c ecc.EdDSACurve) (priv *PrivateKey, err error) {
	priv = new(PrivateKey)
	priv.PublicKey.curve = c
	priv.PublicKey.X, priv.D, err = c.GenerateEdDSAFromSeed(seed)
	return
}
