...
// Returns nil for both return values if there is no valid primary identity.
...
// Identities and subkeys are re-signed in case they changed since NewEntity.
...
func (e *Entity) serializePrivate(w io.Writer, config *packet.Config, reSign bool) (err error) {
	if e.PrivateKey == nil {
		return goerrors.New("private key is missing")
	}
...
func checkKeyRequirements(usedKey *packet.PublicKey, config *packet.Config) error {
	...
	case packet.PubKeyAlgoEd25519, packet.PubKeyAlgoX25519:
		// Modern curve encodings; nothing to check beyond the algorithm itself.
	}
	return nil
}
