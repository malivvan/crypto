...
type EncryptedKey struct {
	...
	encryptedMPI1      encoding.Field // used for ECDH
	encryptedMPI2      encoding.Field // used for ECDH
	ephemeralPublicEcc []byte         // used for X25519
	encryptedSession   []byte         // used for X25519
}
...
func (e *EncryptedKey) parse(r io.Reader) (err error) {
	...
	default:
		return errors.UnsupportedError("unsupported algorithm for encryption")
	}
	if e.Version < 6 {
		switch e.Algo {
		case PubKeyAlgoX25519:
...
func (e *EncryptedKey) Decrypt(priv *PrivateKey, config *Config) error {
	...
	case PubKeyAlgoECDH:
		...
		if sk, ok := priv.PrivateKey.(*ecdh.PrivateKey); ok {
			b, err = ecdh.Decrypt(sk, vsG, m, oid, fp)
		} else if d, ok := priv.PrivateKey.(X25519Decrypter); ok {
			// Hardware-backed key: compute the shared secret externally.
			var shared []byte
			shared, err = d.DecryptX25519(vsG[1:])
			if err == nil {
				pub, ok := priv.PublicKey.PublicKey.(*ecdh.PublicKey)
				if !ok {
					err = errors.InvalidArgumentError("unexpected ECDH public key type")
				} else {
					b, err = ecdh.DecryptFromSharedSecret(shared, pub, m, oid, fp)
				}
			}
		} else {
			err = errors.InvalidArgumentError("unsupported ECDH private key type")
		}
	case PubKeyAlgoX25519:
		if sk, ok := priv.PrivateKey.(*x25519.PrivateKey); ok {
			b, err = x25519.Decrypt(sk, e.ephemeralPublicEcc, e.encryptedSession)
		} else if d, ok := priv.PrivateKey.(X25519Decrypter); ok {
			// Hardware-backed key: compute the shared secret externally.
			var shared []byte
			shared, err = d.DecryptX25519(e.ephemeralPublicEcc)
			if err == nil {
				pub := priv.PublicKey.PublicKey.(*x25519.PublicKey)
				b, err = x25519.DecryptWithSharedSecret(e.ephemeralPublicEcc, pub.Point, shared, e.encryptedSession)
			}
		} else {
			err = errors.InvalidArgumentError("unsupported X25519 private key type")
		}
	default:
	...
	case PubKeyAlgoX25519:
		if e.Version < 6 {
			...
			case CipherAES128, CipherAES256:
				break
			default:
				return errors.StructuralError("v3 PKESK mandates AES as cipher function for x25519")
			}
		}
		key = b[:]
	default:
...
func SerializeEncryptedKeyAEADwithHiddenOption(w io.Writer, pub *PublicKey, cipherFunc CipherFunction, aeadSupported bool, key []byte, hidden bool, config *Config) error {
	...
	// In v3 PKESKs, for X25519, mandate using AES
	if version == 3 && cipherFunc != CipherAES128 && cipherFunc != CipherAES256 {
		switch pub.PubKeyAlgo {
		case PubKeyAlgoX25519:
			return errors.InvalidArgumentError("v3 PKESK mandates AES for x25519")
		default:
			break
		}
	}
	...
	case PubKeyAlgoECDH:
		fp := pub.Fingerprint
		if pub.Version == 5 {
			// For v5, the fingerprint must be restricted to 20 bytes (as on the decrypt side).
			fp = fp[:20]
		}
		return serializeEncryptedKeyECDH(w, config.Random(), buf[:lenHeaderWritten], pub.PublicKey.(*ecdh.PublicKey), keyBlock, pub.oid, fp)
	case PubKeyAlgoX25519:
	...
	case PubKeyAlgoEdDSA, PubKeyAlgoEd25519:
