...
func (sig *Signature) Sign(h hash.Hash, priv *PrivateKey, config *Config) (err error) {
	...
	case PubKeyAlgoEdDSA:
		...
		} else if signer, ok := priv.PrivateKey.(crypto.Signer); ok {
			// Hardware-backed signer (e.g. a YubiKey).
			var sig []byte
			sig, err = signer.Sign(config.Random(), digest, crypto.Hash(0))
			if err == nil && len(sig) == ed25519.SignatureSize {
				r, s = sig[:32], sig[32:]
			} else if err == nil {
				err = errors.InvalidArgumentError("invalid EdDSA signature size from signer")
			}
		} else {
			err = errors.InvalidArgumentError("unsupported EdDSA private key type")
		}
		if err == nil {
	...
	case PubKeyAlgoEd25519:
		...
		if sk, ok := priv.PrivateKey.(*ed25519.PrivateKey); ok {
			...
			// Hardware-backed signer (e.g. a YubiKey).
			signature, err = signer.Sign(config.Random(), digest, crypto.Hash(0))
		} else {
			err = errors.InvalidArgumentError("unsupported Ed25519 private key type")
		}
		if err == nil {
			sig.EdSig = signature
		}
...
func (sig *Signature) Serialize(w io.Writer) (err error) {
	...
	if sig.EdDSASigR == nil && sig.EdSig == nil {
...
func (sig *Signature) buildSubpackets(config *Config) (subpackets []outputSubpacket, err error) {
	...
	if sig.IssuerKeyId != nil && sig.Version == 4 {
		...
		// See: https://github.com/malivvan/crypto/issues/263
