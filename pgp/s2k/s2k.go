...
// specified in RFC 4880 section 3.7.1, and Argon2 specified in
// RFC 9580 section 3.7.1.4.
// Simple S2K is parsed for legacy compatibility but is never generated;
// key derivation for new data uses SHA-256 or SHA-512 with the salted
// variants (Salted S2K and Iterated-and-Salted S2K) or Argon2 S2K.
package s2k // import "github.com/malivvan/crypto/pgp/s2k"
...
import (
	...
	"github.com/malivvan/crypto/argon2"
...
// Defines the S2KMode constants
//
//	0 (simple), 1 (salted), 3 (iterated and salted), 4 (argon2)
const (
	SimpleS2K         Mode = 0
	SaltedS2K         Mode = 1
...
type Params struct {
	...
	// It can be 0 (simple), 1 (salted), 3 (iterated and salted), 4 (argon2)
	// 2 (reserved) 100-110 (private/experimental).
	mode Mode
	// hashId is the ID of the hash function used in any of the modes
	// except Argon2.
	hashId byte
	// saltBytes is a byte array to use as a salt in hashing process,
	// or as the Argon2 salt.
	...
	// See RFC 9580 Section 3.7.1.4.
	passes byte
	// parallelism is a parameter in Argon2 to determine the degree of parallelism
	// See RFC 9580 Section 3.7.1.4.
	parallelism byte
...
// function (RFC 9580, section 3.7.1.4).
func Argon2(out []byte, in []byte, salt []byte, passes uint8, parallelism uint8, memoryExp uint8) {
	key := argon2.IDKey(in, salt, uint32(passes), decodeMemory(memoryExp), parallelism, uint32(len(out)))
	copy(out[:], key)
}

// Generate generates valid parameters from given configuration.
// It will enforce the Iterated and Salted or Argon2 S2K method, unless the
// passphrase is known to be high-entropy and Salted S2K is configured.
func Generate(rand io.Reader, c *Config) (*Params, error) {
	...
	if c != nil && c.Mode() == Argon2S2K {
		...
		if c != nil && c.PassphraseIsHighEntropy && c.Mode() == SaltedS2K {
			params.mode = SaltedS2K
			params.countByte = 0
		}
	}
...
func (params *Params) salt() []byte {
	...
	case SaltedS2K, IteratedSaltedS2K:
		return params.saltBytes[:saltSize]
	case Argon2S2K:
...
func (params *Params) Function() (f func(out, in []byte), err error) {
	...
	if params.mode != Argon2S2K {
		var ok bool
		hashObj, ok = algorithm.HashIdToHash(params.hashId)
		if !ok {
...
// validateArgon2Params checks that the argon2 parameters are valid according
// to RFC 9580.
