...
// recover added to the session handler: the ptyallocate used to be closed on the line
// after the handler returned, so recovering a panic would skip it and leak the
// ptyallocate on every panicking session. Trading a crash for a resource leak is not a
...
func TestSessionHandlerPanicClosesPty(t *testing.T) {
	...
	if !closed {
		t.Error("ptyallocate cleanup was skipped when the handler panicked")
	}
}
