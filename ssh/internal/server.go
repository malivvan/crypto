...
type Permissions struct {
	...
	// (sk-ssh-ed25519@openssh.com). When present, it waives the default
	// requirement that SK signatures
...
func NewServerConn(c net.Conn, config *ServerConfig) (*ServerConn, <-chan NewChannel, <-chan *Request, error) {
	...
	if len(fullConf.PublicKeyAuthAlgorithms) == 0 {
		...
		for _, algo := range fullConf.PublicKeyAuthAlgorithms {
			if !slices.Contains(SupportedAlgorithms().PublicKeyAuths, algo) {
				c.Close()
...
func isAlgoCompatible(algo, sigFormat string) bool {
	// The underlying algorithm must match the signature format.
	...
	for {
		...
		case "publickey":
			...
			if isQuery {
				...
				// ssh-ed25519-cert-v01@openssh.com algorithm with an
				// ssh-ed25519 public key type. The algorithm and public key
				// type must be consistent: both must be certificate
				// algorithms, or neither.
