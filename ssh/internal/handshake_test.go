...
func TestHandshakeRekeyDefault(t *testing.T) {
	clientConf := &ClientConfig{
		Config: Config{
			Ciphers: []string{CipherAES256GCM},
		},
		HostKeyCallback: InsecureIgnoreHostKey(),
	}
	trC, trS, err := handshakePair(clientConf, "addr", false)
...
func TestHandshakeAEADCipherNoMAC(t *testing.T) {
	for _, cipher := range []string{CipherChaCha20Poly1305, CipherAES256GCM} {
		checker := &syncChecker{
...
func TestMultiAlgoSignerHandshake(t *testing.T) {
	algorithmSigner, ok := testSigners["ed25519"].(AlgorithmSigner)
	if !ok {
		t.Fatal("ed25519 test signer does not implement the AlgorithmSigner interface")
	}
	multiAlgoSigner, err := NewSignerWithAlgorithms(algorithmSigner, []string{KeyAlgoED25519})
	if err != nil {
	...
	clientConf := &ClientConfig{
		...
		HostKeyCallback:   FixedHostKey(testSigners["ed25519"].PublicKey()),
		HostKeyAlgorithms: []string{KeyAlgoED25519},
	}
...
func TestMultiAlgoSignerNoCommonHostKeyAlgo(t *testing.T) {
	...
	// the client only supports sk-ssh-ed25519
	clientConf := &ClientConfig{
		User:              "test",
		Auth:              []AuthMethod{Password("testpw")},
		HostKeyCallback:   FixedHostKey(testSigners["cert"].PublicKey()),
		HostKeyAlgorithms: []string{KeyAlgoSKED25519},
	}
...
func TestPickIncompatibleHostKeyAlgo(t *testing.T) {
	...
	signer := pickHostKey([]Signer{multiAlgoSigner}, KeyAlgoSKED25519)
	if signer != nil {
...
func TestStrictKEXMixed(t *testing.T) {
	...
	msg := &kexInitMsg{
		...
		ServerHostKeyAlgos:      []string{CertAlgoED25519v01},
	}
	packet := Marshal(msg)
