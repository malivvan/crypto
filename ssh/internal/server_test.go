...
func TestClientAuthRestrictedPublicKeyAlgos(t *testing.T) {
	for _, tt := range []struct {
		...
		{"ed25519", testSigners["ed25519"], false},
		{"sk-ed25519", testSigners["rsa"], false},
		{"dsa", testSigners["dsa"], false},
	} {
		c1, c2, err := netPipe()
		if err != nil {
		...
		serverConf := &ServerConfig{
			PublicKeyAuthAlgorithms: []string{KeyAlgoED25519},
...
func TestNewServerConnValidationErrors(t *testing.T) {
	serverConf := &ServerConfig{
		PublicKeyAuthAlgorithms: []string{"ssh-rsa-cert-v01@openssh.com"},
	}
	c := &markerConn{}
...
func TestPublicKeyCallbackLastSeen(t *testing.T) {
	...
	serverConf := &ServerConfig{
		PublicKeyCallback: func(conn ConnMetadata, key PublicKey) (*Permissions, error) {
			lastSeenKey = key
			fmt.Printf("seen %#v\n", key)
			if _, ok := key.(ed25519PublicKey); !ok {
	...
	expectedPublicKey := testSigners["rsa"].PublicKey().Marshal()
	lastSeenMarshalled := lastSeenKey.Marshal()
	if !bytes.Equal(lastSeenMarshalled, expectedPublicKey) {
		t.Errorf("unexpected key: got %#v, want %#v", lastSeenKey, testSigners["rsa"].PublicKey())
	}
}
...
func TestVerifiedPublicKeyCallback(t *testing.T) {
	...
	serverConf := &ServerConfig{
		VerifiedPublicKeyCallback: func(conn ConnMetadata, key PublicKey, permissions *Permissions, signatureAlgorithm string) (*Permissions, error) {
			...
			if signatureAlgorithm != KeyAlgoED25519 {
				t.Errorf("expected signature algorithm: %q; got: %q", KeyAlgoED25519, signatureAlgorithm)
			}
			return permissions, nil
		},
		PublicKeyCallback: func(conn ConnMetadata, key PublicKey) (*Permissions, error) {
...
func TestVerifiedPublicKeyCallbackSignatureFormat(t *testing.T) {
	...
	if gotSignatureAlgorithm != KeyAlgoED25519 {
		t.Errorf("signature algorithm = %q; want %q", gotSignatureAlgorithm, KeyAlgoED25519)
	}
}
...
// skEd25519TestSigner produces sk-ssh-ed25519 signatures over user-auth
// data, simulating a FIDO/U2F authenticator. The flags byte is
// caller-controlled so tests can exercise the server's user-presence
// enforcement and its opt-out paths.
type skEd25519TestSigner struct {
	priv  ed25519.PrivateKey
	pub   PublicKey
	flags byte
}

func (s *skEd25519TestSigner) PublicKey() PublicKey { return s.pub }

func (s *skEd25519TestSigner) Sign(r io.Reader, data []byte) (*Signature, error) {
	h := sha256.New()
	h.Write([]byte("ssh:"))
	appDigest := h.Sum(nil)
	h.Reset()
	...
	sig := ed25519.Sign(s.priv, Marshal(blob))
	return &Signature{
		Format: KeyAlgoSKED25519,
		Blob: Marshal(struct {
...
// public-key client and verifies the server's wiring of the UP check and
// its opt-out path.
func TestServerAuthSKUserPresence(t *testing.T) {
	...
	skPub := &skEd25519PublicKey{application: "ssh:", PublicKey: pub}
	...
	t.Run("non-SK signer unaffected", func(t *testing.T) {
		if err := runAuth(t, testSigners["rsa"], nil); err != nil {
			t.Errorf("plain ed25519 auth must still work: %v", err)
		}
	})
}
