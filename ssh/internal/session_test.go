...
func dial(handler serverType, t *testing.T) *Client {
	...
	go func() {
		...
		conf.AddHostKey(testSigners["cert"])
...
func newServerShell(ch Channel, in <-chan *Request, prompt string) Channel {
...
func readLine(ch Channel, t *testing.T) {
	var buf [32]byte
	for {
		n, err := ch.Read(buf[:])
...
func TestHostKeyAlgorithms(t *testing.T) {
	...
	// By default, we get the preferred algorithm, which is the ed25519
	// host key certificate.

	clientConf := &ClientConfig{
		HostKeyCallback: InsecureIgnoreHostKey(),
	}
	connect(clientConf, CertAlgoED25519v01)

	// Client asks for the ed25519 cert explicitly.
	clientConf.HostKeyAlgorithms = []string{CertAlgoED25519v01}
	connect(clientConf, CertAlgoED25519v01)
