...
func (kex *mlkem768WithCurve25519sha256) Client(c packetConn, rand io.Reader, magics *handshakeMagics) (*kexResult, error) {
	...
	copy(peerPub[:], reply.EphemeralPubKey[mlkem.CiphertextSize768:])
	var c25519Secret x25519.Key
	if !x25519.Shared(&c25519Secret, (*x25519.Key)(&c25519kp.priv), &peerPub) {
		return nil, errors.New("ssh: peer's mlkem768x25519 public value is not valid")
	}
...
func (kex *mlkem768WithCurve25519sha256) Server(c packetConn, rand io.Reader, magics *handshakeMagics, priv AlgorithmSigner, algo string) (*kexResult, error) {
	...
	var peerPub x25519.Key
	copy(peerPub[:], kexInit.ClientPubKey[mlkem.EncapsulationKeySize768:])
	var c25519Secret x25519.Key
	if !x25519.Shared(&c25519Secret, (*x25519.Key)(&c25519kp.priv), &peerPub) {
		return nil, errors.New("ssh: peer's ML-KEM768/curve25519 public value is not valid")
	}
