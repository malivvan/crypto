		...
		t.Run(name, func(t *testing.T) {
			...
			for i := 0; i < 3; i++ {
				...
				go func() {
					...
					go func() {
						r, e := kex.Server(b, rand.Reader, &magics, testSigners["ed25519"].(AlgorithmSigner), testSigners["ed25519"].PublicKey().Type())
						b.Close()
						s <- kexResultErr{r, e}
					}()
	...
	for name, kex := range kexAlgoMap {
		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				...
				go func() {
					r, e := kex.Server(t2, rand.Reader, &magics, testSigners["ed25519"].(AlgorithmSigner), testSigners["ed25519"].PublicKey().Type())
					t2.Close()
					s <- kexResultErr{r, e}
				}()
