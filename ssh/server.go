...
type Server struct {
	...
	PtyHandler                    PtyHandler                    // ptyallocate allocation handler, ssh.emulatePtyHandler if nil
...
func (srv *Server) Serve(l net.Listener) error {
	...
	for {
		...
		if e != nil {
			...
			// Temporary accept error: back off and retry, mirroring the
...
// isTemporaryAcceptError reports whether e is a temporary network error for
// which Accept should be retried after a backoff.
func isTemporaryAcceptError(e error) bool {
	ne, ok := e.(net.Error)
	return ok && ne.Temporary() //nolint:staticcheck // SA1019: same pattern as net/http
}
